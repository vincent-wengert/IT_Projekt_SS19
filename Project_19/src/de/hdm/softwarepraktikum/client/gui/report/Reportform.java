...
public class Reportform {
	...
	private Label infoTitleLabel = new Label("Report auswÃ¤hlen");

	private Grid selectionGrid = new Grid(3, 5);

	private Label fromLabel = new Label("Zeitraum von");
	private Label toLabel = new Label("bis");
	private Label storeLabel = new Label("Laden auswÃ¤hlen");
	private Label groupLabel = new Label("Gruppe auswÃ¤hlen");
	private Label personLabel = new Label("EinkÃ¤ufe auf mich beschrÃ¤nken");
	...
	private Boolean getIntervallDefined() {
		if (fromDateBox.getValue() == null && toDateBox.getValue() == null) {
			...
			Notification.show("Bitte wÃ¤hlen sie Start- und Endzeitpunkt aus");
			return false;
		} else if (fromDateBox.getValue() != null && toDateBox.getValue() != null) {
