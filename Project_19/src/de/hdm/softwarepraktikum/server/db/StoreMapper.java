...
public class StoreMapper {
	...
	/**
	 * Die Klasse StoreMapper wird nur einmal instantiiert. Man spricht hierbei von
	...
	public Store findByID(int ID) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(
					"SELECT Store_ID, Name, Street, Postcode, City, Creationdate, Changedate, Housenumber FROM Store "
							+ " WHERE Store_ID = " + ID);
			while (rs.next()) {
	 ...
	 * Diese werden als einzelne <code>Store</code> Objekte innerhalb einer
	 * ArrayList zurückgegeben.
	 * 
	...
	// Methode prüft ob Listitems aus einer Gruppe mit bestimmtem Händler in DB
	// vorhanden
	...
	public boolean checkforExisitingStores(Store s) {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT Responsibility_ID FROM Responsibility WHERE Store_ID = " + s.getId());
